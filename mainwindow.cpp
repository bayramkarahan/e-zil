...
{
     zilKontrol();
    // qDebug()<<"ZilBaşlama"<<QTime::currentTime().toString("hh:mm:ss");;
   /// qDebug()<<"Zaman Kontol Ediliyor.."<<currentsaatState<<currentsaat;
   /* if(player->state()==QMediaPlayer::StoppedState)
    {
       // if(currentsaatState!=currentsaat)
        {
            /// qDebug()<<"Stop Zil Kontol Ediliyor..";
            currentsaatState="";
            zilKontrol();
        }
  ...
  /*  if(tenefusYayin==true)
    {
        /// qDebug()<<"stop tenefüs Zil Kontol..";//<<player->state();
        zilKontrol();
    }*/
}
void MainWindow::zilKontrol()
{
    ...
    if(lineayar!="")
   {
     //  qDebug()<<">>"<<QTime::currentTime().toString("hh:mm:ss")<<trn<<gun<<currentsaat<<tenefusYayin<<lineayar.left(3)<<lineayar.left(2);
        ...
        if(lineayar.split("|")[0]=="muzikson")player->stop();
